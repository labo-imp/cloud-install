...
int files_lastactivity(int fullscan, time_t  t0)
{
    ...
    static const char * dires[] = { "/dev/pts", ".virtual_documents", ".local/share/rstudio", 
      ".local/share/jupyter", ".vscode", ".vscode-R", ".config/Code", "labo2025ros" };
         ...
         )
       {
          ...
          if( (ahora - ultimos.gnome) > parametros[0] )
          {
               ...
               if( (ahora - ultimos.files) > parametros[0]  )
               {
                   ...
                   sprintf( st_apagar, "/home/%s/install/apagar-vm.sh", env_user );
